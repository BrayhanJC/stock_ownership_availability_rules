...
class StockConfigSettings(models.TransientModel):
	...
	@api.multi
	def set_default_location_id(self):
		if self.env.user._is_admin():
			IrValues = self.env['ir.values'].sudo()
		else:
			IrValues = self.env['ir.values']
		IrValues.set_default('stock.config.settings', 'location_id', self.location_id.id)
	...
	def set_default_location_dest_id(self):
		if self.env.user._is_admin():
			...
			IrValues = self.env['ir.values']
		IrValues.set_default('stock.config.settings', 'location_dest_id', self.location_dest_id.id)
...
class StockReserve(models.TransientModel):
	...
	def reserve_sales_stock(self):
		...
		source_location = self.company_id.location_id and self.company_id.location_id.id
		destination_location = self.company_id.location_dest_id and self.company_id.location_dest_id.id
		if source_location and destination_location:
			...
			body_html = """ Dear """+ self.sale_order.partner_id.name + """,<br />We are pleased to inform you that stock from <b>"""+ self.sale_order.name + """</b>
							as per bellow list is reserved by <b>""" + self.env.user.name +"""</b>.<br /><br />"""
			...
			if move_lines:
				...
				immediate_transfer = self.env['stock.immediate.transfer'].create({'pick_id': picking.id})
				picking.action_assign()
				immediate_transfer.process()
				body_html += """</table><br />
								Do not hesitate to contact us if you have any question. <br /><br />
								Thanks & Regards.<br />
								from """+ self.sale_order.partner_id.company_id.name + """.
							</p>
						</div> """	
				email_cc_list = []
				for customer in self.notify_user:
					email_to = formataddr((customer.name or 'False', customer.email or 'False'))
				...
				values['res_id'] = False
				values['subject'] = "Stock Reservation Confirmed : " + self.sale_order.name
